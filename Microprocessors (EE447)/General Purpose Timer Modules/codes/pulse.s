;Pulse.s
; Routine for creating a pulse train using interupts
...
;---------------------------------------------------
LOW EQU 0x00000028 ; 160 cycle*1000ns = 40us
HIGH EQU 0x0000000A; 40 cycle*1000ns = 10us
;---------------------------------------------------
					...
					EXPORT PULSE_INIT
;---------------------------------------------------
; to generate a square pulse train with a 20KHz frequency
; Timer generates 1us clocks and enter interrupt in each 1us.
; 20Khz --> 50us --> 10us HIGH + 40us LOW
My_Timer0A_Handler  PROC
	; R8: current state and determines the next state, init: LOW
					CMP R8,#LOW
					BEQ write_high
					BNE is_low

write_high			LDR R9 ,=TIMER0_TAILR             
					LDR R8 ,=HIGH
					STR R8,[R9]
led_on				LDR R9,=GPIO_PORTF_DATA
					MOV R7,#0xFFFF
					STR R7,[R9]
					B finish		
...
write_low			LDR R9 ,=TIMER0_TAILR              
					LDR R8 ,=LOW
					STR R8,[R9]
led_off				LDR R9,=GPIO_PORTF_DATA
					MOV R7,#0x0000
					STR R7,[R9]
					B finish

finish 				LDR R9,=TIMER0_ICR
					MOV R7,#0x01
					STR R7,[R9]
					BX LR
					ENDP
...
PULSE_INIT PROC
			...
			LDR R0 , [R1]
			ORR R0 , R0 , #0x04 ; set bit 2 for output      
			STR R0 , [R1]
...
; Interupts 16-19 are handled by NVIC register PRI4.                     
...
; set NVIC interrupt 19 to priority 2.              
			...
			ORR R2 , R2 , #0x40000000 ; set interrupt priority to 2.                   
			STR R2 , [ R1 ]

; NVIC has to be enabled  
; Interrupts 0-31 are handles by NVIC register EN0                  
